...
const csvFilePath = 'contrib/moonkin-items.csv'
const supplementalFilePath = 'contrib/supplemental-items.csv'
...
class ConvertItem {
  ...
  get wowHeadURL(): string {
    ...
    const encodedName = encodeURIComponent(this.itemBaseName)
  ...
  get itemOnUse(): ItemOnUseJSON | undefined {
    const tooltip = this._wowHeadItem['htmlTooltip'][0]
    if (tooltip.includes('Use:')) {
      const $ = cheerio.load(tooltip)
      const onUseText = $('span .q2').html()
      $('a[class=q2]').remove()
      const tmpstr = $('span[class=q2]').html()
      const onUseCooldown = tmpstr.substring(tmpstr.lastIndexOf('(') + 1, tmpstr.lastIndexOf(')'))
      return {
        effect: onUseText,
...
const start = async function () {
  ...
  const myArray: ItemJSON[] = []

  for (const ogObj of jsonArray) {
    if (ogObj.Name === '') {
      continue
    }
